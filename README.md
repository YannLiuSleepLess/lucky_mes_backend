...
* Check the `ConnectionStrings` in `appsettings.json` files under the `AbpDemo.HttpApi.Host` and `AbpDemo.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 712cc5ef-6a34-4944-8cac-4c9d7abf76cb
```

> `712cc5ef-6a34-4944-8cac-4c9d7abf76cb` is the password of the certificate, you can change it to any password you want.
...
* `AbpDemo.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
